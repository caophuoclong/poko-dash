...
const RESERVED_NAMESPACES = new Set([
  '$json', 'trigger', 'input', 'secrets', 'env',
  'workflow', 'system', 'loop', 'previous', '$node',
])
...
export function validateWorkflowVariableKey(
  ...
  if (!VALID_KEY_RE.test(key)) {
    errors.push('Key must start with a letter or underscore, and contain only letters, digits, and underscores')
  }
  ...
  if (RESERVED_NAMESPACES.has(key)) {
    errors.push(`"${key}" is a reserved namespace and cannot be used as a variable key`)
  }

  // Non-blocking: recommend UPPER_SNAKE_CASE
  if (errors.length === 0 && !SNAKE_CASE_UPPER_RE.test(key)) {
    warnings.push('Recommended: use UPPER_SNAKE_CASE (e.g. MY_KEY) for workflow variables')
  }
 ...
 */
export function validateWorkflowVariableValue(_value: string): { valid: boolean; errors: string[] } {
