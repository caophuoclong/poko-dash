...
export function TextFieldEditor({ schema, value, onChange, errors, availableVars }: PropertyEditorProps) {
  const inputRef = useRef<HTMLInputElement>(null)
  const [pickerOpen, setPickerOpen] = useState(false)
  const [pickerAnchor, setPickerAnchor] = useState<{ top: number; left: number } | undefined>()

  const handleDragOver = useCallback((e: React.DragEvent) => {
    if (e.dataTransfer.types.includes('application/variable-ref') || e.dataTransfer.types.includes('text/plain')) {
      e.preventDefault()
