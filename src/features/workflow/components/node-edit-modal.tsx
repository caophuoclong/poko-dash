...
export function NodeEditModal({
  ...
  // console.log('🚀 ~ NodeEditModal ~ prevExecInfo:', prevExecInfo)

  const upstreamData = useMemo(() => {
    if (!prevNode) return null
    const synthesized = synthesizeOutput(prevNode as Node<WorkflowNodeData>)
    if (
      prevExecInfo &&
      prevExecInfo.status === 'completed' &&
      prevExecInfo.outputSummary
    ) {
      return { ...synthesized, ...prevExecInfo.outputSummary }
    }
    return synthesized
        ...
        <div className="flex-1 grid grid-cols-[minmax(240px,1fr)_minmax(380px,1.4fr)_minmax(240px,1fr)] divide-x divide-frost overflow-hidden min-h-0">
          ...
          <div className="flex flex-col bg-surface-2/20 overflow-hidden">
            <PaneHeader
              ...
              subtitle={
                prevNode
                  ? `Connect an upstream node to see its output`
                  : undefined
              }
            ...
            {prevNodes.length > 1 && (
              <div className="flex items-center justify-center gap-1 px-4 py-1.5 border-b border-frost bg-surface">
                <button
                  onClick={() =>
                    setPrevIdx(
                      (i) => (i - 1 + prevNodes.length) % prevNodes.length,
                    )
                  }
